import React from 'react';
import './header.css';
import people from '../../assets/people.png'
import ai from '../../assets/ai.png'

const Header = () => {
    return (
        <div className="site__header section__padding" id="home">
            <div className="site__header-content">
                <h1 className="gradient__text">Let's Build Something Amazing with GPT OpenAI</h1>
                <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Autem eum error corrupti veritatis explicabo deleniti earum illum quas totam cupiditate nihil eligendi, dignissimos nulla nam rerum? Beatae sit dolor unde!</p>
                <div className="site__header-content__input">
                    <input type="email" placeholder="Your Email Address"></input>
                    <button type="button">Get Started</button>
                </div>
                <div className="site__header-content__people">
                    <img src={people} alt="people" />
                    <p>1,600 people requested access in the last 24 hours</p>
                </div>
            </div>
            <div className="site__header-img">
                    <img src={ai} alt="ai"/>
            </div>
        </div>
    )
}

export default Header